...
from agents import (
    Agent, Runner, ModelProvider,
    Model, RunConfig, OpenAIChatCompletionsModel, set_tracing_disabled
)
...
class CustomModelProvider(ModelProvider):
    def get_model(self, model_name: str | None) -> Model:
        return OpenAIChatCompletionsModel(model="gpt-5-mini",
...
You write concise, to the point cold emails."
        self._sales_agent_1 = Agent(name="Professional Sales Agent",
                                    instructions=self._sales_agent_1_instructions)
        self._sales_agent_2 = Agent(name="Humorous Sales Agent",
                                    instructions=self._sales_agent_2_instructions)
        self._sales_agent_3 = Agent(name="Concise Sales Agent",
                                    instructions=self._sales_agent_3_instructions)

    async def run_single_agent(self, agent: Agent, input: str, run_config: RunConfig) -> str:
    ...
    async def run_all_agents(self, input: str, run_config: RunConfig):
        ...
        async with aiofiles.open("outputs/sales_agent_outputs.txt", "w", encoding="utf-8") as f:
...
class SimpleSalesAgentSystem():
    ...
    def check_sendgrid_client(self) -> bool:
        status_code = send_test_email("Check SendGrid Client",
